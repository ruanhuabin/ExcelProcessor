...
import os
import logging
from LipidIonProcessor import initRangeValue, run_lipid_process
class ScreenFrame:
    def __init__(self, master):
        ...
        self.ms2WindowEntry.delete(0, END)
        self.ms2WindowEntry.insert(0, "0.2")
        
        
        self.blankText3 = StringVar()
        self.blankLabel3 = Label(frame,textvariable = self.blankText2).grid(row = 16, columnspan = 2)
        
        self.topRTRangeLabel = Label(frame, text="TopRTRange:          ")
        self.topRTRangeLabel.grid(row=17, columnspan=2)
       
        self.topRTRangeEntry = Entry(frame, width=20, bd = 4)
        self.topRTRangeEntry.grid(row=18,columnspan=2)
        
        self.topRTRangeEntry.delete(0, END)
        self.topRTRangeEntry.insert(0, "0.25")

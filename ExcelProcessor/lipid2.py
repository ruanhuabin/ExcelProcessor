...
if __name__ == '__main__':
    ...
    logger.info("compndNames = " + str(compndNames))   
    
    compndNames = list(compndNames)
    compndNames = sorted(compndNames)
    allc = open("allcompounds.txt", 'w')
    allc.truncate()
    for n in compndNames:
        allc.write(n + "\n")
    ...
    logger.info("cg dict after removing: " + str(cgDict))
...
#     将cfDict中不再compndName中的化合物删掉
    ...
    logger.info("cf dict after removing: " + str(cfDict))
